...
const stegosaurus = { diet: 'herbivorous', weight: '2000kg', length: '9m', period: 'Late Jurassic' };
// velociraptor, carnivorous, 15kg, 1.8m, Late Cretaceous
const velociraptor = { diet: 'carnivorous', weight: '15kg', length: '1.8m', period: 'Late Cretaceous' };
...
const universities = graduates.map(({ university }) => university).sort();
...
const uni = graduates.filter(({ university }) => university.match(/(uni)/gi)).map(({ university }) => university);
console.log(uni);
...
zooAnimals.forEach(({ animal_name, scientific_name }) =>
	animalNames.push(`${animal_name}, Scientific: ${scientific_name}.`),
);
...
*/
const populationTotal = zooAnimals.reduce((acc, animals) => (acc += animals.population), 0);
